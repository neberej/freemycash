...
const Savings: React.FC = () => {
  ...
  const monthlyData = data.transactions.reduce((acc, transaction) => {
    ...
    if (transaction.type === 'income') acc[monthYear].income += transaction.amount;
    else acc[monthYear].expenses += transaction.amount;
    return acc;
  ...
  useEffect(() => {
    ...
    const handler = (e: TouchEvent) => {
      ...
      if (points.length > 0) {
        ...
        chartInstance.tooltip.setActiveElements(points, { x, y });
